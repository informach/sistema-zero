...
describe('leitor de backup do Molda ("Trazer de volta")', () => {
  const json = galleryToJsonText([makeSky()], 123)

  it('abre o ZIP e lê somente diretório, cabeçalho e JSON, nunca a entrada grande ignorada', async () => {
    ...
    const zip = zipSync(
      {
        'modelos/nave.glb': ignored,
        [MOLDA_GALLERY_ZIP_ENTRY]: strToU8(json),
        'LEIA-ME.txt': strToU8('olá'),
      },
      { level: 0 },
    )
    const file = new TrackingFile([ownedBuffer(zip)], 'minhas-criacoes-3d-molda.zip', {
      type: 'application/zip',
    })

    const result = await readMoldaBackupFile(file)
  ...
  it('abre de volta o ZIP realmente produzido por "Baixar tudo"', async () => {
    const bytes = await zipGallery([makeTexture({ name: 'grama-do-molda' })], {
      yieldBetween: null,
    })
    const result = await readMoldaBackupFile(
      new File([ownedBuffer(bytes)], 'minhas-criacoes-3d-molda.zip', { type: 'application/zip' }),
    )
  ...
  it('explica ZIP inválido, sem backup e com entrada duplicada', async () => {
    ...
    ).toEqual({ ok: false, reason: 'missing-backup' })

    const duplicate = await zipWithDuplicateEntry(MOLDA_GALLERY_ZIP_ENTRY, json, json)
    expect(
      await readMoldaBackupFile(
        new File([ownedBuffer(duplicate)], 'duplicado.zip', { type: 'application/zip' }),
      ),
    ).toEqual({ ok: false, reason: 'duplicate-backup' })
  })

  it('interrompe a entrada descompactada assim que ela passa do teto', async () => {
    const zip = zipBytes({ [MOLDA_GALLERY_ZIP_ENTRY]: '123456789' })
  ...
  it('recusa a entrada canônica criptografada e marcadores ZIP64', async () => {
    const zip = zipBytes({ [MOLDA_GALLERY_ZIP_ENTRY]: json })
