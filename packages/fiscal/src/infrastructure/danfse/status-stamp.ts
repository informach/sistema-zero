 ...
 * do gerador do governo. Uma falha deve subir para a borda HTTP: servir uma nota
 * cancelada ou substituída sem a marca d'água seria uma representação incorreta.
 */
...
export async function applyStatusStamp(
  ...
  fonts: DanfseFontFiles = loadBundledDanfseFontFiles(),
): Promise<Uint8Array> {
  const pdf = await PDFDocument.load(bytes, { ignoreEncryption: true })
  pdf.registerFontkit(fontkit)
  const font = await pdf.embedFont(fonts.arialRegular, { subset: true })
