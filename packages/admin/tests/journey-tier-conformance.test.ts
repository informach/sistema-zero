...
describe('conformance admin×core — degraus e posições da Jornada do Criador', () => {
  ...
  test('slotsForTier ≡ requiredSlots da Lenda (nº de posições por etapa)', () => {
    ...
    for (const option of COURSE_TIER_OPTIONS) {
      const tier = `${option.level}-${option.track}` as (typeof JOURNEY_COURSE_TIERS)[number]
  ...
  test('só a ENTRADA foge do 8, e nenhuma passa do teto do core', () => {
    ...
    // Iniciante 2D teve 7 entre 14/08 e 15/08 e a usuária desfez. O `JOURNEY_SLOT_MAX`
    ...
    for (const option of COURSE_TIER_OPTIONS) {
      ...
      expect(slots).toBeLessThanOrEqual(JOURNEY_SLOT_MAX)
    }
  })
