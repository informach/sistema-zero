...
import { CAREER_COURSE_TIERS, CREATOR_CAREER_LEVELS } from '../../core/src/career/catalog'
...
describe('conformance admin×core — degraus e posições da Carreira do Criador', () => {
  test('COURSE_TIER_OPTIONS ≡ CAREER_COURSE_TIERS (conjunto E ORDEM)', () => {
    // A ORDEM é load-bearing: o CareerReadiness deriva "6 posições" de `index === 0`
    // (1ª opção precisa ser iniciante-2d).
  ...
  test('regra do CareerReadiness (index 0 → 6 posições) ≡ slotsForTier', () => {
    // O painel usa `index === 0 ? 6 : 5` sobre COURSE_TIER_OPTIONS — trava a
    // equivalência com a função do form (mesma fonte de verdade operacional).
    COURSE_TIER_OPTIONS.forEach((option, index) => {
      expect(slotsForTier(option.level, option.track)).toBe(index === 0 ? 6 : 5)
    })
  })
})
