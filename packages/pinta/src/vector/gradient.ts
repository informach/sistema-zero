...
export function gradientGeometry(gradient: VectorGradient): GradientGeometry {
  ...
  return {
    type: 'linear',
    start: validPoint(gradient.start) ? gradient.start : { x: axis.x1, y: axis.y1 },
    end: validPoint(gradient.end) ? gradient.end : { x: axis.x2, y: axis.y2 },
  }
}
...
export function moveGradientHandle(
  ...
  const normalized = { x: clamp(point.x, 0, 1), y: clamp(point.y, 0, 1) }
  ...
  if (handle === 'center') return { ...gradient, center: normalized }
  if (handle === 'radius') {
    const geometry = gradientGeometry(gradient)
