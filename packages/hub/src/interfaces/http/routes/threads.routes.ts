...
export function threadsRoutes(deps: ThreadsRoutesDeps) {
  return (
    new Elysia()
      ...
      // Tópicos do PRÓPRIO aluno (sino "novas respostas") — literal, não colide com
      // `/hub/threads/:id`. Só devolve os próprios → sem redação a fazer.
      .get('/hub/my-threads', ({ headers }) => deps.threads.listMyThreads(resolveActor(headers)))
      .get(
        '/hub/threads/:id',
