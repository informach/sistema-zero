...
/**
 * Inicializa o Sentry (erros apenas — sem tracing/logs). Sem DSN é no-op: todas as
 * chamadas `Sentry.capture*` viram no-op seguro. Espelha o padrão dos demais serviços.
 */
export function initSentry(opts: SentryOptions): boolean {
  ...
  Sentry.init({
    ...
    // Comunidade: nunca anexar headers/IP/cookies por default — os logs já
    // minimizam PII (userId em vez de e-mail) e o Sentry não pode virar o vazamento.
 ...
 * evento. A convenção do package é "log ERROR = sinal alertável". Agrupamento por
 * NOME do evento (fingerprint); contexto vira `extra`. Observabilidade NUNCA
