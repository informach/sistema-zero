...
- **Exclusão pelo painel (`POST …/:id/deletion/prepare` + `DELETE /auth/admin/users/:id`,
  endurecida em 19/08/2026):** o prepare usa o mesmo advisory lock da criação de perfil,
  bloqueia a conta e captura **todos** os perfis (inclusive arquivados) antes da purga externa;
  uma conta não ativa não cria perfil novo. A chamada final faz a limpeza de contas de
  ...
  **`audit_logs` é PRESERVADA** (compliance, `actor_id` snapshot). Guards (defesa em profundidade,
  ...
  chamada via gateway — financeiro/fiscal são RETIDOS. Rotas no gateway:
  `auth-admin-user-deletion-prepare` e `auth-admin-user-delete` (superadmin +
  `authInternalTransforms` + `audit`).
