 ...
 * padrão do payments (validado em produção).
 */
export function initSentry(opts: SentryOptions): boolean {
  ...
  Sentry.init({
    ...
    // Serviço de IDENTIDADE: nunca anexar headers/IP/cookies por default — os
    // logs já minimizam PII por convenção (userId em vez de e-mail) e o Sentry
    // não pode virar o vazamento.
 ...
 * evento. A convenção do package é "log ERROR = sinal alertável" (e-mail de
 * reset/OTP que não saiu, purga falhando...) — com o espelho, esses sinais
 * geram issue/alerta sem tocar nenhum call site. Agrupamento por NOME do evento
 * (fingerprint); contexto vira `extra`. Observabilidade NUNCA derruba o caminho
 * principal (capture em try/catch).
 */
