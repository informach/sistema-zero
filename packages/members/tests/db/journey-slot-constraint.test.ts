  ...
  async function accepts(row: Probe): Promise<boolean> {
    try {
      await sql.begin(async (tx) => {
        await tx.unsafe(`
          create temporary table journey_slot_probe (
            audience text not null,
            level text not null,
            track text not null,
            career_slot smallint,
            constraint journey_slot_probe_check check (${checkExpression})
          ) on commit drop`)
        await tx`
          insert into journey_slot_probe (audience, level, track, career_slot)
