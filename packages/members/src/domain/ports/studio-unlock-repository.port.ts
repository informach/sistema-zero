 ...
 * cursos que o aluno concluiu + publicou, e essa união é calculada AO VIVO sobre o
 * `metadata.studioUnlockBlocks` de cada curso — o que faz um bloco ACRESCENTADO ao JSON
 * chegar sozinho em quem já concluiu (desejado), mas também faria um bloco REMOVIDO (ou
 * um curso despublicado/apagado) sumir da mão de quem já o tinha, inclusive de projetos
 * que já o usam. O snapshot é o piso que impede isso.
 *
...
export interface StudioUnlockRepository {
  ...
  /**
   * Congela (ou amplia) o que um curso deu ao aluno. O chamador já resolveu a UNIÃO —
   * aqui é escrita direta, idempotente por (aluno, vitrine, curso).
   */
  saveGrants(
    userId: string,
