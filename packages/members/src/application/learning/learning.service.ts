...
/** Lê o que está guardado na forma da cena certa. Formato errado devolve `null`, e quem chama
 *  trata isso como sessão INEXISTENTE: a cena recomeça limpa e a gravação nova substitui a linha. */
...
export class LearningService {
  ...
  async save(
    ...
    if (cena) {
      ...
      // ⚠️⚠️ Sessão guardada que NÃO hidrata (retrato truncado, adulterado ou incoerente) vale
      // como INEXISTENTE: a cena recomeça limpa e esta gravação substitui a linha. Tratá-la como
      // conflito trancava o bloco para sempre — 409 em toda gravação, recarregar não adiantava
      // (a linha continuava no banco) e não havia caminho de cura dentro do produto. O conflito
      // de VERDADE (sessão válida de outra aba, pedido fora de ordem, segmento repetido com outro
      // conteúdo) continua abaixo, e a gravação segue presa à linha que ESTE pedido leu.
      const sequenciaNaLinha =
        saved && typeof saved.answers.sceneSequence === 'number'
          ? saved.answers.sceneSequence
          : null
      ...
      // que recusar este. ⚠️ Sem sessão legível a base é ZERO, que é onde o player também
      // recomeça: ele lê o checkpoint com a MESMA régua do servidor.
      if (segment.baseSequence !== (guardado?.checkpoint.sequence ?? 0))
        throw new LearningConflictError()
      // ⚠️ A trava otimista aponta para a linha que este pedido LEU, não para o que foi
      // hidratado: assim a linha ilegível é substituída, mas uma gravação concorrente que
      // entrar no meio ainda derruba esta (a sequência no banco deixa de ser a lida).
      expectedExperienceSequence = guardado?.checkpoint.sequence ?? sequenciaNaLinha
      try {
        answers = {
          ...applySceneSegment(cena, guardado, segment),
          segmentHash: hash,
        }
      } catch (error) {
