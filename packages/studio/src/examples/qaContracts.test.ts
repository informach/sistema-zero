...
describe('contrato transversal dos 124 exemplos da KitGallery', () => {
  it('catálogo e contratos têm exatamente as mesmas chaves', () => {
    ...
    expect(catalogKeys).toHaveLength(124)
    expect(new Set(catalogKeys).size).toBe(124)
  })

  it('mantém a classificação acordada: 103 jogos, 9 demos e 12 explorações', () => {
    const counts = { game: 0, demo: 0, exploration: 0 }
    for (const contract of EXAMPLE_QA_CONTRACTS) counts[contract.experience] += 1
    expect(counts).toEqual({ game: 103, demo: 9, exploration: 12 })
  })
