...
/** SHA-256 síncrono em base64, usado pelas fontes de hash da CSP e pelo SRI. */
export function sha256Base64(text: string): string {
  ...
  for (let offset = 0; offset < paddedLength; offset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temp1 = (h + big1 + choice + (SHA256_ROUND[index] ?? 0) + (words[index] ?? 0)) >>> 0
...
export function scriptIntegrity(code: string): `sha256-${string}` {
  return `sha256-${sha256Base64(code)}`
}
