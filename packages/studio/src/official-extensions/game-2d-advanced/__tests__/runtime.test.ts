...
import { GAME_KIT_ENUMERABLE_API_KEYS, type GameKitRuntimeApi } from '../runtimeContract'
...
type TypedTestMethods = {
  ...
  navePowerOf: (character: unknown) => string
  pathProgress: (character: unknown) => number
  pickActive: (mold: string, mode: string, property: string) => unknown
}
type GenericTestApi = { [Key in keyof GameKitRuntimeApi]: Fn }
type GameKitApi = Omit<GenericTestApi, keyof TypedTestMethods> & TypedTestMethods
interface Harness {
  api: GameKitApi
...
describe('SZGameKit — API e personagens (sem DOM)', () => {
  it('expõe o inventário completo de métodos usados pelos blocos', () => {
    const { api, inspectors } = loadRuntime()
    const expected = GAME_KIT_ENUMERABLE_API_KEYS
...
describe('SZGameKit — telas (happy-dom) e laço', () => {
  it('start monta o palco + 5 telas prontas e termina no menu', async () => {
    ...
    expect(canvas?.getAttribute('aria-label')).toBe(
      'Use as setas, WASD, espaço, enter ou o mouse conforme as instruções do jogo.',
    )
