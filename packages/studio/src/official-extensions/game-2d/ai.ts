...
KIT EQUILIBRISTA (decomposto na v0.40.0) — categoria "🤸 Kit equilibrista", jogo estilo "Stick Hero" (estica o bastão e atravessa) montado PASSO A PASSO pela criança. Controle pelo PONTEIRO global (segurar = esticar, soltar = derrubar) — NÃO precisa registrar listeners. Canvas em pé (ex.: 360×480). O loop recomendado no "a cada quadro": clear → stickHeroScenery → stickHeroHold → stickHeroStep → stickHeroDraw → HUD (drawScore) → se caiu, showScreen.
- stickHeroCreate(ctx, corHeroi, corBastao, corPlataformas): monta o jogo com as cores da criança e guarda numa variável (ex.: jogo). Faça UMA vez, fora do "a cada quadro".
- stickHeroScenery(jogo): desenha céu, colinas e árvores. stickHeroHold(jogo, rapidez): estica o bastão enquanto o ponteiro segura e derruba ao soltar. stickHeroStep(jogo, rapidez): a física (andar pelo bastão, atravessar ou cair). stickHeroDraw(jogo): desenha plataformas, bastão e herói com as cores escolhidas.
- stickHeroOnCross(jogo, fn, id) / stickHeroOnPerfect(jogo, fn, id): eventos (atravessou / acertou a faixa do meio, que vale 2 pontos). Registrar UMA vez, fora do "a cada quadro".
- stickHeroSetShape(jogo, "figura") / stickHeroSetImage(jogo, "imagem"): o herói vira um desenho da criança — figura do defineShape (grupo Desenho) ou imagem do projeto (ex.: desenho do Pinta). Caixa QUADRADA (lado = altura nativa do herói, ancorada no pé): dentro da figura use shapeW()/shapeH(). Nome vazio volta ao boneco pronto. Última chamada vence (figura×imagem, uma coisa por vez).
- stickHeroScore(jogo): valor — pontos. stickHeroOver(jogo): valor (booleano) — caiu? restartStickHero(jogo): zera o jogo (bom com "Quando apertar Enter"). Exemplo pronto "Equilibrista" já traz o loop montado.
- LEGADO (ocultos da paleta, só para projetos salvos): createStickHero(ctx) e updateStickHero(jogo) — o update faz o passo INTEIRO (física + desenho + HUD + recomeço por toque).

KIT BALÃO (decomposto na v0.40.0) — categoria "🎈 Kit balão", jogo estilo "Hot-Air-Balloon" (sobe segurando, economiza combustível, desvia das árvores) montado PASSO A PASSO. Segurar o ponteiro = aquecer/subir (gasta combustível); voar baixo economiza. Canvas deitado (ex.: 560×360). Loop recomendado: clear → balloonScenery → balloonLift → balloonScroll → balloonDraw → HUD (drawScore/drawBar) → se acabou, showScreen.
- balloonCreate(ctx, corBalao, corCesto): monta o jogo com as cores da criança e guarda numa variável. Faça UMA vez.
- balloonScenery(jogo): desenha céu, colinas, chão e árvores. balloonLift(jogo, forca): sobe enquanto segurar (gasta combustível; sem combustível o balão pousa e acaba). balloonScroll(jogo, velocidade): avança o mundo, conta os metros e confere a batida nas árvores. balloonDraw(jogo): desenha o balão com o cesto e a chama.
- balloonOnTreeHit(jogo, fn, id): evento da batida na árvore (explosão/tremida/som). Registrar UMA vez, fora do "a cada quadro".
- balloonSetShape(jogo, "figura") / balloonSetImage(jogo, "imagem"): o balão vira um desenho da criança — figura do defineShape ou imagem do projeto. A caixa é o retângulo do balão nativo (2R de largura por 3R de altura, mais alta que larga). Nome vazio volta ao balão pronto.
- balloonScore(jogo): valor — metros voados. balloonFuel(jogo): valor — combustível 0..100. balloonOver(jogo): valor (booleano) — bateu numa árvore ou acabou o combustível? restartBalloon(jogo): zera o jogo. Exemplo pronto "Balão" já traz o loop montado.
- LEGADO (ocultos da paleta, só para projetos salvos): createBalloon(ctx) e updateBalloon(jogo).
