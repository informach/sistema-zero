...
describe('game-2d — todos os exemplos da vitrine (manifest.examples)', () => {
  ...
  it('a vitrine tem os 23 exemplos (anti-vácuo)', () => {
    expect(gameTwoDManifest.examples.length).toBe(23)
  })
  ...
  it('mantém a classificação prometida: 18 jogos, 4 demonstrações e 1 exploração', () => {
    const counts = { game: 0, demo: 0, exploration: 0 }
    for (const example of gameTwoDManifest.examples) counts[example.experience] += 1
    expect(counts).toEqual({ game: 18, demo: 4, exploration: 1 })
  })
