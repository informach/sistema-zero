...
} from '../codecs/programming/irToBlock'
...
import {
  gameTwoDActionExpressionToBlock,
  gameTwoDActionToBlock,
...
import { withPeriodicKey } from '../official-extensions/game-2d/periodicCodec'
import {
...
export type { SerializedBlocklyBlock } from '../codecs/types'
...
function statementsToBlocks(
  ...
  return statements.flatMap((statement) => {
    const serialized = statementToBlock(statement)
    if (!isBlock(serialized)) return []
    ...
    if (!placement || placement.root.length === 0 || placement.nested.length > 0)
      return [serialized]
    // Código fora dos encaixes visuais continua como código editável. O editor
    // atual não cria invólucros de compatibilidade para encaixes inexistentes.
    return [rawJSBlock(statement)]
  })
}
...
function statementToBlockInner(stmt: JSStatement): SerializedBlocklyBlock | null {
  ...
  const programmingBlock = programmingStatementIRToBlock(stmt, {
  ...
  function gameTwoDStatementToBlock(): SerializedBlocklyBlock {
    const action = gameTwoDActionToBlock(stmt, {
      block,
    ...
    switch (stmt.type) {
      ...
      case 'g2d:stopTrack':
        return block('sz_g2d_stop_track', { SCOPE: stmt.scope ?? 'all' }, {}, stmt.__id)
      case 'g2d:setVolume': {
      ...
      case 'g2d:everyFrames': {
        ...
        return withPeriodicKey(
          block('sz_g2d_every_frames', {}, { BODY: statementsToBlocks(stmt.body) }, stmt.__id, {
            N: n,
          }),
          stmt.key,
        )
      ...
      case 'gk:rpgCreateMap': {
        ...
        return cols === null || rows === null
          ...
          : block(
              ...
              { MAP: stmt.map, PARAM: stmt.ctxName, BOUNDS: stmt.bounds ?? 'bounded' },
              { BODY: statementsToBlocks(stmt.body) },
              stmt.__id,
              { COLS: cols, ROWS: rows },
            )
 ...
 * tem preset de sombra para o slot (fonte: a própria paleta) e o filho é o
...
function shouldEmitAsShadow(blockType: string, slot: string, child: SerializedBlocklyBlock) {
  const type = paletteShadowType(blockType, slot)
  if (!type || !['sz_val_number', 'sz_val_text', 'sz_val_color'].includes(type)) return false
...
function exprToValueBlockInner(expr: JSExpr): SerializedBlocklyBlock | null {
  ...
  if (canvasBlock !== undefined) return canvasBlock
  const programmingBlock = programmingExpressionIRToBlock(expr, {
    ELEMENT_EVENT_BLOCK_TYPES,
    NAMED_ELEMENT_EVENT_KINDS,
    STYLE_PROP_VALUES,
    WINDOW_EVENT_BLOCK_TYPES,
  ...
  switch (expr.type) {
    ...
    case 'g2d:levelIsActive':
      return block('sz_g2d_level_is_active', { LEVEL: expr.levelVar })
    case 'g2d:stickPathFell':
      return block('sz_g2d_stickpath_fell', { PATH: expr.pathVar })
    case 'g2d:balloonPathMeters':
      return block('sz_g2d_balloonpath_meters', { PATH: expr.pathVar })
    case 'g2d:balloonFuel':
      return block('sz_g2d_balloon_fuel_left', { SPRITE: expr.spriteVar })
    case 'g2d:balloonLandedOut':
      return block('sz_g2d_balloon_landed_out', { SPRITE: expr.spriteVar })
    case 'g2d:pointerDown':
      return block('sz_g2d_pointer_down', {})
    ...
    case 'g3d:countGroup':
      return block('sz_g3d_count_group', { GROUP: expr.groupVar })
    case 'g3d:isNear': {
