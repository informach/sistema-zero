...
import { getWebContainer, resetWebContainerFs } from '../../components/terminal/webContainerClient'
import { computeFsDiff } from './fsDiff'

export type WcMountState = 'idle' | 'mounting' | 'mounted' | 'error'
...
export function useWebContainerSync(snapshot: TerminalProjectSnapshot): UseWebContainerSyncResult {
  ...
  useEffect(() => {
    ...
    if (diff.writes.length === 0 && diff.removes.length === 0) return
