...
import {
  collectGameTwoDActionIdentifiers,
  gameTwoDActionToCode,
...
function compileStatementCode(
  ...
  function gameTwoDStatementToCode(): string {
    const tools: Parameters<typeof textSpriteStatementToCode>[1] = {
      pad,
    ...
    const extensionCode =
      gameTwoDActionToCode(stmt, tools) ?? textSpriteStatementToCode(stmt, tools)
    if (extensionCode !== undefined) return extensionCode
    ...
    switch (stmt.type) {
      ...
      case 'g2d:stopTrack':
        return `${pad}SZGame2D.stopTrack(${stmt.scope ? JSON.stringify(stmt.scope) : ''});`
      case 'g2d:setVolume':
  ...
  ${pad}});`
      ...
      case 'g2d:everyFrames': {
        ...
        const key = stmt.key ?? identifiers.reserveInternal('cada')
        return [
      ...
      case 'g2d:afterSeconds': {
        ...
        const key = stmt.key ?? identifiers.reserveInternal('depois')
        return [
...
function collectExprIdentifiers(expr: JSExpr, names: Set<string>): void {
  ...
  switch (expr.type) {
    ...
    case 'gk:didHit':
      ...
      names.add(expr.bVar)
      return
    // 🎬 R18 — dois nomes: quem gira e para quem
    case 'gk:angleTo':
      names.add(expr.charVar)
      names.add(expr.targetVar)
      return
    case 'gk:nearestActive':
      collectExprIdentifiers(valueToExpr(expr.x), names)
      collectExprIdentifiers(valueToExpr(expr.y), names)
      return
    // Sem variável nenhuma: o molde/item/round são strings literais.
    case 'gk:countItem':
    case 'gk:lutaWinner':
    case 'gk:lutaRound':
    ...
    case 'gk:healthOf':
    // 🎬 R18 — o "quem" destes é uma VARIÁVEL: sem coletar, o gerador renomeia errado
    case 'gk:animEnded':
    case 'gk:entityState':
    case 'gk:angleOf':
    // 🥊 R19
