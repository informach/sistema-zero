...
function compileStatementCode(
  ...
  switch (stmt.type) {
    ...
    case 'g2d:playNote':
      return `${pad}SZGame2D.playNote(${JSON.stringify(stmt.note)}, ${stmt.ms});`
    case 'g2d:aimAt':
    ...
    case 'g2d:setTile':
      return `${pad}SZGame2D.setTileAtSprite(${identifiers.get(stmt.mapVar)}, ${stmt.index}, ${identifiers.get(stmt.spriteVar)});`
