...
export function compileExpr(
  ...
  switch (expr.type) {
    ...
    case 'colorAlpha': {
      const r = Number.parseInt(expr.hex.slice(1, 3), 16)
      const g = Number.parseInt(expr.hex.slice(3, 5), 16)
      const b = Number.parseInt(expr.hex.slice(5, 7), 16)
      const a = Math.round(expr.alpha * 100) / 100
