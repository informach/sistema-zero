...
import { dueloNaMaoExample } from '../../examples/core'
import { parseProjectFilesWithDiagnostics } from '../index'

/**
 * ⭐ "na mão" — o fighting-game do Chris Courses (Downloads/Fight/fighting-game)
 * reescrito 100% com blocos do NÚCLEO, sem extensão. Dois lutadores no mesmo
 * teclado, gravidade + chão, golpe com CAIXA DE GOLPE à frente (overlap tira 15
 * de vida), barras de vida DESENHADAS no canvas (o "na mão" no lugar do
 * gsap.to('#health')), cronômetro regressivo e a tela de vencedor. Asset-free
 * (retângulos), construtores com parâmetros simples, canvas dedicado por ctx.
 */
...
function animate() {
  ...
  ctx.fillText('Tempo: ' + timer, 260, 40);

  if (over) {
    ctx.fillStyle = '#ffd166';
    ctx.font = '32px sans-serif';
    ctx.fillText(winner, 230, 160);
  }
}
animate();
...
describe('parsers — duelo "na mão" (núcleo, sem extensão)', () => {
  ...
  it('drift: a IR embutida no exemplo da vitrine é EXATAMENTE a que o parser gera', () => {
    ...
    expect(normalize(dueloNaMaoExample.ir)).toEqual(normalize(ir))
  })

  it('fixpoint textual: gerar → parsear → gerar é byte-estável', () => {
    const ir1 = parseProjectFilesWithDiagnostics(GAME_FILES).ir
    const files1 = generateProjectFiles({ ir: ir1, projectName: 'Duelo' })
    const second = parseProjectFilesWithDiagnostics(files1)
    expect(second.diagnostics).toEqual([])
    expect(collectTypes(second.ir).has('rawJS')).toBe(false)
    const files2 = generateProjectFiles({ ir: second.ir, projectName: 'Duelo' })
  ...
  it('round-trip por BLOCOS: IR → workspace → IR → código byte-igual', () => {
    ...
    try {
      ...
      const filesFromBlocks = generateProjectFiles({ ir: irFromBlocks, projectName: 'Duelo' })
