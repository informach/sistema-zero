...
/**
 * ⭐ "na mão" — o platformer kings-and-pigs do Chris Courses (Downloads/Games 2D/
 * kings-and-pigs-main: classes CollisionBlock/Player/Sprite) reescrito 100% com
 * blocos do NÚCLEO, sem extensão. Além da física (gravidade + colisão AABB
 * resolvida SEPARADAMENTE em x e em y), o que ele ENSINA de novo é a passagem
 ...
 * direito"):
 *  - A PORTA agora abre quando o herói ENCOSTA nela E aperta PARA CIMA (W), fiel
 *    ao original. Antes o fade disparava SOZINHO só de encostar na porta, o que
 *    brigava com o W (que também é o pulo): o herói teleportava de fase sem
...
function atDoor() {
  return overlap(player.x, player.y, player.width, player.height, doorX, doorY, 26, 40);
}
...
function animate() {
  ...
  if (fadeDir === 0 && atDoor()) {
    ctx.fillText('Aperte para cima para entrar', 180, 30);
  }
...
describe('parsers — portas do castelo "na mão" (platformer com fase, núcleo)', () => {
