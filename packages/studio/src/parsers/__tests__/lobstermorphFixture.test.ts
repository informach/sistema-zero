...
/**
 * FIXTURE REAL: o "Lobstermorph V9" (Franks Laboratory, Enemy Class) — bem mais
 * avançado que o Space Invaders (V1): HERANÇA com super(...) e super.metodo(),
 * spritesheets desenhadas com drawImage de 9 argumentos, imagens da página por
 * document.getElementById, save/restore/fillText/sombras no canvas e um laço de
 * jogo COM tempo entre quadros (requestAnimationFrame à mão). O contrato: o jogo
 * INTEIRO vira blocos do NÚCLEO (0 rawJS/rawCSS/rawHTML, sem a extensão Jogo 2D),
 * o round-trip é FIXPOINT estável e volta idêntico pelos BLOCOS. Normalizações
 * aceitas (comportamento idêntico): window→document nos listeners de tecla,
 * e→event, ternário-statement → if/senão.
 */

const SCRIPT_JS = `class Laser {
    constructor(game){
        this.game = game;
        this.x = 0;
        this.y = 0;
...
<body>
    ...
    <img src="beetlemorph.png" id="beetlemorph">
    <img src="rhinomorph.png" id="rhinomorph">
    <img src="squidmorph.png" id="squidmorph">
    <img src="eaglemorph.png" id="eaglemorph">
    <img src="lobstermorph.png" id="lobstermorph">
    <img src="enemyProjectile.png" id="enemyProjectile">
    <img src="player.png" id="player">
    <img src="player_jets.png" id="player_jets">
    <img src="boss.png" id="boss">
    <img src="boss8.png" id="boss8">
...
#beetlemorph, #rhinomorph, #lobstermorph, #squidmorph, #eaglemorph, #enemyProjectile, #player, #player_jets, #boss, #boss8 {
    display: none;
}`
...
describe('Lobstermorph V9 (Franks Laboratory) — 100% blocos do núcleo', () => {
  it('parseia os 3 arquivos SEM raw e sem diagnóstico', () => {
    ...
    for (const expected of ['superCall', 'superMethodCall', 'getElement', 'requestFrame']) {
  ...
  it('round-trip textual é FIXPOINT com os marcos do jogo', () => {
    ...
    const files1 = generateProjectFiles({ ir: ir1, projectName: 'Lobstermorph' })
    const second = parseProjectFilesWithDiagnostics(files1)
    expect(second.diagnostics).toEqual([])
    expect(collectTypes(second.ir).has('rawJS')).toBe(false)
    const files2 = generateProjectFiles({ ir: second.ir, projectName: 'Lobstermorph' })
    ...
    expect(jsOut).toContain('class SmallLaser extends Laser')
    expect(jsOut).toContain('super(game)')
    expect(jsOut).toContain('super.render(context)')
    expect(jsOut).toContain('document.getElementById("beetlemorph")')
    expect(jsOut).toContain('.drawImage(')
    expect(jsOut).toContain('requestAnimationFrame(')
    expect(jsOut).toContain('event.key')
    expect(files1['index.html']).toContain('id="beetlemorph"')
