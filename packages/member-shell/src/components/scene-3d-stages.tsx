...
export function Camera3dStage({
  ...
  state: SceneState
  cast?: SceneCast
  /** A prévia não entrega quantas cores a criança vai descobrir ao girar a câmera. */
  ...
  return (
    <SceneCanvas
      ...
      descricao={`A câmera está na volta ${yaw + 1} de 8, ${NOME_DA_ALTURA[pitch] ?? 'no meio'}.${ocultarContagem ? '' : ` Daqui ${vistas === 1 ? 'aparece' : 'aparecem'} ${quantos(vistas, 'cor', 'cores')}.`}`}
      rodape={ocultarContagem ? undefined : 'Lados opostos têm a mesma cor.'}
    >
      {(palco) => (
        <>
