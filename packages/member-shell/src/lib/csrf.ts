 ...
 * dos cookies de sessão. No domínio definitivo a área do aluno divide o eTLD+1
 * com funil/admin/cdn: um subdomínio irmão comprometido dispara requisições
 * **same-site** que LEVAM os cookies do aluno (`Lax` não barra same-site). Por
 * isso TODA mutação precisa ser **same-origin** (espelha o admin):
 *
 ...
 *    mutações são sempre `fetch()`/`sendBeacon` das próprias telas (same-origin).
