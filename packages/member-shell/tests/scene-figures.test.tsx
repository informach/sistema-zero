 ...
 * 2. Um palco que pusesse a moldura no espaço e esquecesse o `FundoEspaco` (a grama da terra
...
const TRACO = {
  dino: /d="M-22 -8V-33H-12V-51/g,
  cacto: /d="M-7 0V-20H-20V-40/g,
  floresta: /d="M-43 -28L-25 -61/g,
}
/** A folhagem da terra. No espaço, só com um cacto ou uma floresta desenhados. */
const FOLHAGEM = /fill-scene-leaf(?:-soft|-dark)?(?![\w-])/g
...
function conferir(scene: SceneId, cast: SceneCast | undefined, htmls: string[]): string[] {
  ...
  if (papeis.length > 0) {
    ...
    const comFundo = htmls.filter((h) => h.includes('data-fundo="espaco"')).length
    if (mundo === 'espaco' && comFundo < htmls.length)
      falhas.push(`${rotulo}: a moldura está no espaço e o palco não pintou o céu`)
    if (mundo === 'terra' && comFundo > 0)
      falhas.push(`${rotulo}: o céu do espaço apareceu na terra`)
    const folhas = contar(htmls, FOLHAGEM)
    if (mundo === 'espaco' && folhas > 0 && !doElenco.has('cacto') && !doElenco.has('floresta'))
...
describe('o desenho veste o elenco', () => {
  ...
  test('⚠️⚠️ e os usos REAIS, como estão nos manifestos (com o caso e o roteiro deles)', () => {
    ...
    const mundos = new Set(USOS.map((u) => sceneWorld(u.activity.cast, u.activity.scene)))
    expect([...mundos].sort()).toEqual(['espaco', 'terra'])
  })
})
...
describe('o conferente REPROVA os esquecimentos que o review mostrou', () => {
  ...
  test('o obstáculo desenhado com o traço cru do cacto, sem a figura', () => {
    const htmls = htmlsDe('lives', NAVE).map(
      (h) =>
        `${h.replace(/<g data-figure="asteroide"/g, '<g')}<path d="M-7 0V-20H-20V-40H-12V-29Z"/>`,
    )
  ...
  test('a moldura no espaço sem o céu, e a grama e a árvore da terra dentro dela', () => {
    ...
    const htmls = htmlsDe('lives', NAVE).map((h) =>
      h
        .replace(/data-fundo="espaco"/g, '')
        .replace('</svg>', '<path class="fill-scene-leaf-soft" d="M-43 -28L-25 -61Z"/></svg>'),
    )
    const falhas = conferir('lives', NAVE, htmls)
    expect(falhas.some((f) => f.includes('não pintou o céu'))).toBe(true)
  ...
  test('o Dino cru no espaço, e o mundo errado na moldura', () => {
    const htmls = htmlsDe('draw-loop', NAVE).map((h) =>
      h
        .replace('data-mundo="espaco"', 'data-mundo="terra"')
        .replace('</svg>', '<path d="M-22 -8V-33H-12V-51Z"/></svg>'),
    )
    const falhas = conferir('draw-loop', NAVE, htmls)
    expect(falhas.some((f) => f.includes('traço de dino'))).toBe(true)
    expect(falhas.some((f) => f.includes('não está no mundo espaco'))).toBe(true)
  })
})
