...
/**
 * Inicializa o Sentry (erros apenas — sem tracing/logs por enquanto, o sinal
 * alertável deste serviço são exceções + logs ERROR). Sem DSN é no-op: todas as
 * chamadas `Sentry.capture*` viram no-op seguro quando não inicializado.
 */
export function initSentry(opts: SentryOptions): boolean {
  ...
  Sentry.init({
    ...
    // Serviço de PAGAMENTOS: nunca anexar headers/IP/cookies por default — os
    // logs já são PII-free por convenção e o Sentry não pode virar o vazamento.
 ...
 * evento. A convenção do package é "log ERROR = sinal alertável" (falha de
 * publicação do outbox, item de webhook, amount mismatch, tick de worker...) —
