...
/**
 * CSP do helpdesk (defesa em profundidade — não há sink de HTML cru no app).
 * Sem nonce: o Next 16 injeta scripts inline de bootstrap e o Tailwind/libs
 * injetam estilos inline → `'unsafe-inline'` é necessário (e em dev o
 * Turbopack/HMR usa `eval`). Fontes externas reais:
 ...
 * Removido em relação ao marketing-app: `connect-src *.r2.cloudflarestorage.com`
 * (este app não tem upload direto browser→R2).
 */
const csp = [
  "default-src 'self'",
  "base-uri 'self'",
