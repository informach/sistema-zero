...
/**
 * Catch-all do BFF p/ o @sistemazero/helpdesk: encaminha `/api/helpdesk/<path>`
 * → gateway `/helpdesk/<path>` com o Bearer do cookie (refresh-on-401 no
...
// decide o RBAC fino). `oauth` cobre só o START da conexão Gmail (o callback é
// navegação do Google direto no gateway, nunca passa por aqui).
const ALLOWED_ROOTS = new Set(['tickets', 'kb', 'settings', 'connection', 'oauth'])
...
async function handle(req: Request, params: Promise<{ path: string[] }>): Promise<NextResponse> {
  ...
  const upstreamPath = `/helpdesk/${path.map(encodeURIComponent).join('/')}`
  const { search } = new URL(req.url)

  const method = req.method as Method
  // Corpo é JSON em todas as mutações do serviço; POST sem corpo segue sem body —
  // o `.catch` cobre o parse de corpo vazio.
