 ...
 *
 * ⚠️ Por que NÃO o SDK `@sentry/node` (como os serviços Elysia)? Os outros
 * serviços rodam no Bun, onde o SDK é um dep normal. AQUI o app é Next App
 * Router com `output: 'standalone'` + Turbopack: o server BUNDLA os deps no
 * chunk, e um pacote `serverExternalPackages` (necessário p/ o SDK do Sentry,
 * que usa `require-in-the-middle`/OpenTelemetry) precisaria ser TRAÇADO p/ o
 * `node_modules` do standalone — o que o tracing do Turbopack não faz de forma
 * confiável (lição do admin). Em vez de depender disso, falamos o protocolo de
 * ingestão do Sentry direto via `fetch` (embutido no runtime) — zero dependência
 * nova, mesmo comportamento em dev/build/container.
 *
 * Política (espelha o princípio do gateway: capturar SÓ o sinal que este app vê
 * de forma única): erros LOCAIS — exceções inesperadas de Route Handler/RSC
 * (via `onRequestError`). 5xx de upstream NÃO são espelhados aqui — o gateway
 * já os captura (evita duplicar).
 *
...
export function buildEnvelope(input: {
  ...
  const event = JSON.stringify({
    ...
    // agrupamento fica por tipo+mensagem, suficiente p/ o volume do app).
