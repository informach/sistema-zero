/**
 * Launcher de PRODUÇÃO do marketing-app (CMD do Dockerfile): valida a env e SÓ
 ...
 * (verificado empiricamente no admin, standalone 16.2.7). A instrumentation
...
if (process.env.NODE_ENV === 'production') {
  ...
  // Prod usa SÓ RS256 via JWKS; um segredo HS256 forjaria a sessão local que
  // autoriza o gate de UI (mantenha em sincronia com src/instrumentation.ts e lib/env.ts).
...
if (problems.length > 0) {
  console.error(`@sistemazero/marketing-app: env de boot inválida — ${problems.join('; ')}`)
  process.exit(1)
}
