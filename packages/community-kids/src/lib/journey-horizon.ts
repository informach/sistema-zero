...
import { LEVEL_STUDY, LEVEL_TIER } from '@/lib/journey-map'
 ...
 * sendo do members/core. Isto é apresentação. Quando o catálogo encher, `journeyHorizon`
 * devolve `god`, `visibleJourneyLevels` devolve os 8 e a visão definitiva volta SOZINHA
 * (travado em `tests/journey-horizon.test.ts`).
 *
 ...
 * `LEVEL_STUDY` já são travados contra o core em `tests/journey-conformance.test.ts`, então esta
 ...
 * nó do mapa (`journey-horizon-node.tsx`) e o aviso de ferramentas do Molda
 ...
 */
export type CatalogInput = readonly JourneyCatalogEntry[] | null
 ...
 */
export function journeyHorizon(courses: CatalogInput): StudentLevelSlug {
 ...
 */
export function visibleJourneyLevels(
...
export type JourneyProgress =
  | {
       ...
       * esperança. Ver o comentário no `journeyProgress`.
       */
      hint: string | null
    }
