/**
 * Geometria PURA da fita do Mapa da Jornada (`/cursos`).
 *
 ...
 * demais do penúltimo).
 */
export const JOURNEY_OFFSETS = [-1, 1, -1, 1, -1, 1, -1, 1] as const

/** % da largura por unidade de offset (metade da amplitude do zigue-zague). */
export const JOURNEY_STEP = 16
/** Unidades de viewBox por linha (só escala relativa; a altura real vem do `--journey-row`). */
export const JOURNEY_ROW = 100

export interface JourneyGradientStop {
...
export interface JourneyGeometry {
  ...
  traveledPath: string
  gradientStops: JourneyGradientStop[]
}

function pointAt(index: number): JourneyPoint {
  const offset = JOURNEY_OFFSETS[index % JOURNEY_OFFSETS.length] ?? 0
  return { x: 50 + offset * JOURNEY_STEP, y: (index + 0.5) * JOURNEY_ROW }
}

/** Curva suave: bézier cúbica com os controles no meio vertical de cada segmento. */
function smoothPath(points: readonly JourneyPoint[]): string {
 ...
 */
export function buildJourneyGeometry(count: number, currentIndex: number): JourneyGeometry {
  ...
  const gradientStops: JourneyGradientStop[] =
  ...
  return {
    points,
    viewWidth: 100,
    viewHeight: Math.max(points.length, 1) * JOURNEY_ROW,
