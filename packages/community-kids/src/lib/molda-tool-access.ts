 ...
 */
export const MOLDA_BEYOND_HORIZON = 'E tem muito mais pela frente'
 ...
 * dele vira `MOLDA_BEYOND_HORIZON`, e grupos seguidos com a mesma frase se juntam.
 */
export function moldaToolAccessFor({
  ...
  const open = MOLDA_TOOL_BANDS.filter((band) =>
  ...
  for (const band of MOLDA_TOOL_BANDS) {
    ...
    const beyond = LEVEL_ORDER.indexOf(level as StudentLevelSlug) > LEVEL_ORDER.indexOf(horizon)
