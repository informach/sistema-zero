...
export function register(): void {
  ...
  if (prod) {
    ...
    // rotas de mídia/downloads (mantenha em sincronia com o refine do lib/env.ts).
  ...
  if (problems.length > 0) {
    throw new Error(`@sistemazero/community-kids: env de boot inválida — ${problems.join('; ')}`)
  }
}

/**
 * Hook nativo do Next 16: captura TODA exceção inesperada de Route Handler/RSC
 * para o Sentry (erros que VIRAM resposta 500 sem lançar — ex.: a pipeline de
 * mídia — são capturados no próprio `mediaErrorResponse`). Import dinâmico p/ não
 * carregar nada no caminho do `register()`/boot; falha de captura nunca propaga.
 */
