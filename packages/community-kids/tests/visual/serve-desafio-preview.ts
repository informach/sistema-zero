...
const [bundle, styles] = await Promise.all([
  Bun.build({
    entrypoints: [resolve(import.meta.dir, 'desafio-preview.tsx')],
    target: 'browser',
    minify: true,
...
const server = Bun.serve({
  hostname: '127.0.0.1',
  port: Number(process.env.DESAFIO_PREVIEW_PORT ?? 4323),
  fetch(request) {
    ...
    return new Response(
      '<!doctype html><html lang="pt-BR"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Ensaio Desafio v6</title><link rel="stylesheet" href="/app.css"><body><div id="root"></div><script type="module" src="/app.js"></script></body></html>',
