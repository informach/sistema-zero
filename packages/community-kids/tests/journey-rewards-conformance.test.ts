...
const textOf = (slug: (typeof CREATOR_JOURNEY_LEVELS)[number]['slug']) => {
  const info = JOURNEY_REWARD_INFO[slug]
  return `${info.title} ${info.description}`
}
const firstWith = (pick: (r: (typeof CREATOR_JOURNEY_LEVELS)[number]['reward']) => boolean) =>
  CREATOR_JOURNEY_LEVELS.find((level) => pick(level.reward))?.slug

/** O posto exato em que um portão abre, lido da constante que o código usa de verdade. */
const levelOf = (slug: string) => CREATOR_JOURNEY_LEVELS.find((level) => level.slug === slug)?.slug
 ...
 */
describe('journey-rewards (kids) — a copy não promete o que a regra não dá', () => {
  test('nunca promete Ponte a quem não tem Ponte', () => {
    const mentiras = CREATOR_JOURNEY_LEVELS.filter(
  ...
  test('nenhum posto promete kit de blocos — quem entrega bloco é o CURSO', () => {
    const mentiras = CREATOR_JOURNEY_LEVELS.filter((level) =>
  ...
  test('o Pinta é anunciado no posto de criação livre, e em nenhum outro', () => {
    const anunciam = CREATOR_JOURNEY_LEVELS.filter((level) =>
  ...
  test('o Molda é anunciado no posto da oficina 3D e nos postos das faixas, e em nenhum outro', () => {
    ...
    expect(anunciam).toEqual(
      CREATOR_JOURNEY_LEVELS.filter((level) => postos.has(level.slug)).map((level) => level.slug),
    )
  ...
  test('todo nível da escada tem copy (nível novo no core não sai mudo)', () => {
    for (const level of CREATOR_JOURNEY_LEVELS) {
      const info = JOURNEY_REWARD_INFO[level.slug]
