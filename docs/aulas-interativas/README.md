...
A revisão pedagógica de 12/09/2026 organiza essas aulas em **96 seções**, com três a cinco etapas por aula. Abertura, descoberta e explicação ficam juntas; passos de construção próximos formam uma conquista; teste, personalização e entrega se encontram no fechamento. Foram retirados 143 cartões de perguntas redundantes. Os 118 trechos de demonstração continuam previstos, na mesma ordem técnica. Veja o [mapa da revisão e validação em staging](qa/revisao-pedagogica-2026-09-12.md).
...
Os critérios selecionados em cada seção usam uma conferência no servidor. A hipótese inicial pode estar errada; a checagem final precisa ser acertada. Sequências, associações e experiências pedem uma conclusão coerente com o que foi observado. Elas não concedem XP nem desbloqueiam ferramentas por conta própria. Abrir uma seção, salvar um rascunho ou assistir a um vídeo não conta como conclusão. Vários textos e vídeos podem compartilhar uma única checagem. Em 16 etapas, a criança usa **Verificar esta etapa** no próprio Estúdio, com 19 objetivos estruturais ao todo. A última seção também precisa cumprir seu critério.
...
4. Se já importou uma versão anterior deste pacote, confira a lista **Cartões antigos a retirar após reimportação** no fim de cada roteiro e retire esses cartões do rascunho. O importador preserva blocos omitidos e pode levar as perguntas antigas ao fechamento. Confira separadamente os quizzes e entregas da aula original e selecione os que realmente devem ser critérios. Materiais opcionais, incluindo o vídeo longo original, ficam em **Materiais de apoio**, recolhidos fora da sequência. O histórico persistido não deve ser apagado.
