...
Todos os vídeos ainda são `plannedVideo`; portanto, o curso **não está pronto para publicação**. Gravar um vídeo por seção a partir dos respectivos `.roteiro.md`, vincular no admin e conferir a reprodução e a exigência de 90%. O Caderno do Aluno opcional está em `output/pdf/cade-todo-mundo-caderno-do-aluno.pdf`: anexar o mesmo PDF ao bloco `caderno` da segunda seção, **Seu mapa do jogo**, da Aula 1 e ao bloco da seção prática da Aula 2. Depois da capa, ele traz o Mapa do jogo e os passos das duas aulas. O visual e o conteúdo editável estão em `recursos/cade-todo-mundo/caderno-do-aluno.template.html`; para gerar o PDF, executar `bun docs/aulas-interativas/recursos/cade-todo-mundo/gerar-caderno.ts`. Gravar o vídeo curto da segunda seção da Aula 1 depois de anexar o PDF, mostrando uma página verdadeira. A conclusão dessa seção exige só o vídeo; o download não bloqueia a progressão.
