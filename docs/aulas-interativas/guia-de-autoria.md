...
**A ordem é livre para o professor.** Você pode repetir e intercalar exploração e criação conforme o conteúdo. Reutilize a referência ao mesmo espaço de trabalho; não duplique o projeto a cada seção. Cada trecho de criação tem seus próprios objetivos. A entrega fica no momento de fechamento escolhido, antes do quiz final quando houver ambos.
...
O piloto v5 tem 14 missões nas 13 aulas: existir/desenhar, camadas, gravidade, impulso, evento de pulo, ritmo dos cactos, limpeza, estado do jogo, controles, reinício, área de colisão, pontos, sorteio e aceleração. A criança atua na cena, conecta peças, muda a ordem de desenho, move o cacto e redimensiona a área. Há alternativas por toque e teclado, desfazer, pistas e comparação. Camadas e contato respondem imediatamente; o movimento permite pausa, passos e câmera lenta.

A aula 3 exemplifica a composição livre: explorar gravidade → aplicar no projeto → explorar impulso → ajustar o mesmo projeto → entregar → quiz. O avanço confirma cada objetivo separadamente.
