...
> Aula de referência do projeto. As outras 26 análises seguem este formato.
...
- **Vitória do dia:** a nave dela aparece num espaço com estrelas e obedece às setas, sem sair da
  tela.
- **Seções hoje:** 16 · **Seções propostas:** 12
- **Clipes hoje:** 12 · **Clipes propostos:** 10, sendo 2 fusões de pares e 1 clipe novo na entrega
- **Cenas:** 5, todas construídas (4 que já existiam e a `once-vs-always`, feita para este dia)
- **Manifesto:** `aulas/desafio-dia-1.manifesto.json`, 23 blocos e 12 seções. Estado no validador:
...
Onze coisas, seis concretizações. Cinco conceitos não ganham nada, e é essa triagem que tira a
aula de 16 para 12 seções. A proposta pedagógica é de 10 movimentos; duas delas se partem em
duas seções cada por causa da regra das duas colunas do player, explicado em cada caso abaixo.
...
> seção: ou a cena, ou o Estúdio embarcado. Dois movimentos desta aula pediam as duas coisas juntas,
> então viraram duas seções cada. São as seções 2 e 3 (conceito das áreas, depois a montagem delas) e
> as seções 10 e 11 (montagem do desenho da nave, depois o contrafactual das camadas). A intenção
> didática não mudou, e o par continua sendo lido como um movimento só.
...
- **Conclui quando:** as três metas da cena caem e a pergunta final é respondida
- **Blocos:**
  1. `video` (`video-duas-areas`, "O ventilador que não para"). A analogia do ventilador, já gravada
     e boa: "tudo o que estiver dentro dela fica se repetindo sem parar, enquanto o jogo estiver
     ligado. Pensa num ventilador: enquanto está ligado, ele gira, gira, gira, sem parar." Cortar a
     enumeração antiga de passos. Duração alvo: 40 a 50 segundos.
  2. `interactive`. Cena `once-vs-always`, preset `duas-caixas-nave` (especificada abaixo). Elenco:
...
- **Conclui quando:** Ao iniciar e Enquanto estiver rodando estão no projeto, e a tela está preparada
  em 800 por 480 dentro de Ao iniciar
- **Blocos:**
  1. `dialogue`. Orientação de montagem: "Em Áreas do projeto, coloque Ao iniciar e Enquanto
     estiver rodando, com um espaço entre eles. Depois, em Jogo 2D, Jogo e telas, encaixe Preparar
     o jogo em tela cheia dentro de Ao iniciar. Deixe largura 800 e altura 480, e escolha um fundo
     escuro para enxergar a nave e as estrelas."
  2. `video` (`video-montar-areas-tela`, "As áreas do projeto e a tela do jogo"). O gesto das três
...
- **Blocos:**
  ...
  2. `interactive`. Cena `coordinates`, "O endereço na tela". Elenco: nave. Cenário: nave.

### Seção 5. Crie a sua nave

- **Intenção:** construção
- **Por que existe:** é o primeiro objeto do jogo dela, e termina numa surpresa proposital.
- **Conclui quando:** o bloco Criar nave está em Ao iniciar com nome nave, x 400, y 410, largura 54
  e altura 62
- **Blocos:**
  1. `dialogue`. "Em Jogo 2D, Kits prontos › Espaço, encaixe Criar nave logo abaixo de Preparar o
     jogo, dentro de Ao iniciar. Use nome nave, x 400, y 410, largura 54 e altura 62. As cores do
     corpo e das asas são suas."
  2. `video` (`video-criar-nave`, "A nave com as suas cores"). O gesto de pegar e configurar, com a
...
- **Conclui quando:** as duas metas de `world` caem
- **Blocos:**
  1. `video` (`video-livrinho`, "O livrinho de folhear"). A analogia do livrinho de folhear, já
     gravada e excelente: "cada página é um desenho um pouquinho diferente, e passando rápido o seu
     olho enxerga movimento. Cada página dessas tem um nome: quadro." Duração alvo: 35 a 45 segundos.
  2. `interactive`. Cena `world`, "Criar e mostrar são duas coisas diferentes". Elenco: nave.

**Sem mudança de estrutura.** Esta seção já é o modelo correto e serve de referência para as outras.
...
- **Blocos:**
  1. `dialogue`. Abertura curta do Zappy, sem vídeo: "O jogo desenha um quadro, apaga e desenha
     outro, muito rápido. Vamos ver isso acontecendo devagar, um quadro por vez."
  2. `interactive`. Cena `draw-loop`, "Por que o desenho se repete". Elenco: nave. As três metas:
     sem redesenhar a tela não muda, sem limpar ficam os desenhos velhos, limpando e desenhando a
     nave anda.

**Não tem vídeo de propósito.** A cena avança quadro a quadro com o x exposto na faixa, e mostra o
ciclo melhor do que qualquer narração conseguiria. O clipe de hoje vira a abertura em texto.
...
- **Conclui quando:** A cada quadro do jogo está em Enquanto estiver rodando, com Limpar a tela e
  Desenhar fundo de estrelas dentro, nessa ordem, velocidade 1
- **Blocos:**
  1. `dialogue`. "Em Jogo 2D, Tempo › Quadros e intervalos, pegue A cada quadro do jogo e encaixe
     em Enquanto estiver rodando. Dentro dele, encaixe Limpar a tela, em Desenho e efeitos. Logo
     abaixo, Desenhar fundo de estrelas, em Cenários › Fundos, com velocidade 1. A borracha vem
     antes do desenho novo."
  2. `video` (`video-motor-e-fundo`, "A lousa mágica e o espaço com estrelas"). Junta os dois clipes
...
- **Conclui quando:** Mover o sprite nave com as setas (velocidade 7) e Manter o sprite dentro da
  tela estão no motor, os dois apontando para nave
- **Blocos:**
  1. `dialogue`. "Em Jogo 2D, Movimento, coloque Mover o sprite nave com as setas, velocidade 7,
     logo abaixo das estrelas. Depois coloque Manter o sprite dentro da tela. Os dois vêm escritos
     com outro nome de fábrica: clique no nome e troque para nave."
  2. `video` (`video-setas-e-borda`, "As setas e a borda da tela"). O gesto dos dois blocos e, junto,
...
- **Conclui quando:** Desenhar o sprite está no fim do motor, com nave escolhida, depois do fundo e
  do movimento
- **Blocos:**
  1. `dialogue`. "Em Jogo 2D, Sprites, coloque **Desenhar o sprite** no fim de tudo, e escolha
     nave. A ordem do motor fica assim: limpar, estrelas, mover, manter dentro da tela, desenhar a
     nave."
...
> nesta aula pode afirmar que ele é para sempre o último bloco do motor.
  ...
  2. `video` (`video-desenhar-por-ultimo`, "A nave no fim do motor"). O gesto e a ordem final.
...
- **Blocos:**
  1. `interactive`. Cena `layers`, "Quem fica na frente?". Elenco: a nave no papel do herói e o
...
- **Blocos:**
  ...
  2. `quiz`. As duas perguntas atuais, mantidas.
...
**Saldo:** de 12 clipes para 10 blocos de vídeo, sendo 2 fusões de pares (`video-areas` com
`video-tela`, e `video-quadro` com `video-fundo`) e 1 seção (O que é um quadro) que deixa de ter
vídeo. Sai `video-fecho-v6` como clipe próprio: a recapitulação dele vira a fala final do
`video-teste-e-envio`, depois do envio. O teste ganha clipe próprio de novo, agora com o gesto de
enviar dentro dele, pela regra de que toda seção com o Estúdio embarcado tem um vídeo mostrando como
se faz. Em minutagem, a queda é maior do que a contagem sugere, porque as explicações conceituais
