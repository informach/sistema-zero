...
- Backups, revisões antigas e diários foram conservados. Produção não foi alterada. Permanecem: interação visual de remix com a conta de teste já identificada, avaliação pedagógica, regravação/publicação das mídias revisadas e investigação separada da textura 3D que já falhava antes da reforma.
...
### Complemento — homologação autenticada

- A conta ativa `helena@informach.com.br` e o perfil infantil ativo **André Rocha** foram confirmados em staging. O acesso por código temporário foi realizado pelo fluxo público; cookies e tokens ficaram somente em memória.
- Login, seleção do perfil, páginas do Mural e Studio, espaço desbloqueado, canal, seis publicações jogáveis, snapshot em formato 2, lista das 18 criações e permissões de blocos responderam com sucesso. Nenhuma criação foi modificada.
- Limite: o navegador integrado não estava disponível. A rodada autenticada comprovou as fronteiras remotas, mas não o clique visual e a persistência local da cópia. Essa sequência continua pendente numa aba visual autenticada; as provas de navegador do repositório e do CI já cobrem seus componentes sem usar a conta real.
